                            ...
                            <v-row>
                                <v-col v-for="item in items" :key="item.name" cols="12" sm="6" md="4" lg="3">
                                    <v-card>
                                        ...
                                        <v-switch :input-value="isExpanded(item)" :label="isExpanded(item) ? 'Expandded' : 'Closed' " @change="(v) => expand(item, v)"></v-switch>
                                        <v-divider></v-divider>
                                        <v-data-table :headers="headers" :items="desserts" :sort-by="['Username','Score','Playtime']" :sort-desc="[false,true]"></v-data-table>
                                    </v-card>
                                </v-col>
                            </v-row>
                        </template>
